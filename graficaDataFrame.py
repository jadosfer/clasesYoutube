...
def creaDataFrame(caracteristicas, transponerCsv):   
    if not transponerCsv:
        miDataFrame = pd.read_csv("miCsv.csv")
        miDataFrame.columns = caracteristicas
        print(miDataFrame.shape)
        print(miDataFrame.head)
    else:
        miDataFrameT = pd.read_csv("miCsv.csv")
...
def plotDataframe(caracteristicas, titulo, filas, columnas, miUrl, transponerCsv):  
    descargaCreaCsv(miUrl)
    dataFrame = creaDataFrame(caracteristicas, transponerCsv)    
...
#--------------------- llamado a funciones
plotDataframe(caracteristicas, miTitulo, 4, 3, url, True)